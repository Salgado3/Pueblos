...
class Search {
  ...
  async updateField(evt) {
    ...
    this.matches = await fetch(autocompleteGeocodeUrl(value)).then((res) =>
      res.json()
    );
    const match = this.matches.suggestions.filter(
      (x) => x.matchLevel === "city"
    )[0];
    ...
    if (match === undefined) {
      ...
      $(".city-field-suggestion").innerText =
        value + this.label.substring(value.length, this.label.length);
    }
  }
